...
def main() -> None:
    ...
    for threshold in [0.50, 0.55, 0.60, 0.65, 0.70, 0.75]:
        hold = permission_overlay_hold_fraction(aligned, threshold=threshold)
        rows.append(
            evaluate_overlay_variant(
                aligned,
                variant=f"permission_overlay_{threshold:.2f}",
                hold_fraction=hold,
    ...
    write_results("v111_permission_overlay_results.csv", results)
    best = results.iloc[0]
    write_summary(
        "v111_permission_overlay_summary.md",
        "v111 Permission-to-Deviate Overlay",
        [
            "Best candidate:",
            ...
            f"- action month rate: {best['action_month_rate']:.4f}",
        ],
    )
