"""v42 - Expanding window variants: pure, decay-weighted, and capped."""
...
VARIANTS: dict[str, dict[str, int | bool | None]] = {
    "A_pure_expanding": {"max_train_size": None, "decay": False},
    "B_expanding_decay": {"max_train_size": None, "decay": True},
    "C_expanding_cap120": {"max_train_size": 120, "decay": False},
}
MIN_TRAIN_MONTHS = 60
...
def expanding_wfo(
    ...
    """Run an expanding-window Ridge WFO with optional sample weighting."""
    n_obs = len(x_values)
    n_splits = max(1, (n_obs - MIN_TRAIN_MONTHS - GAP_MONTHS) // TEST_SIZE_MONTHS)
...
def main() -> None:
    """Compare expanding-window variants against the rolling baseline."""
    conn = get_connection()
    try:
        ...
        for variant_name, cfg in VARIANTS.items():
            ...
            for etf in BENCHMARKS:
                ...
                y_true, y_hat = expanding_wfo(
                    ...
                    y.to_numpy(),
                    max_train_size=cfg["max_train_size"],  # type: ignore[arg-type]
