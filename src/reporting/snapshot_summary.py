"""Helpers for the shadow-baseline study and monthly snapshot summaries."""
...
from src.models.policy_metrics import hold_fraction_from_policy
...
def build_shadow_comparison_lines(
    ...
    lines = [
        f"# Shadow Comparison - {shadow_summary.as_of.isoformat()}",
        "",
        "## Recommendation",
        "",
        ...
        "- Interpretation: testing whether a simpler baseline policy yields clearer and steadier guidance than the live model stack.",
        "",
...
def build_shadow_check_lines(
    ...
    *,
    ...
    if active_path == "shadow":
        ...
        note_line = (
            "Keeps the live model stack in place, but uses this simpler baseline as a recommendation-layer cross-check because it was steadier in the shadow study."
        )
    return [
