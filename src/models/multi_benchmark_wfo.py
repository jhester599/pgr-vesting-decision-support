...
class EnsembleWFOResult:
    """
    Aggregated result from training ElasticNet + Ridge + BayesianRidge on a
    single ETF benchmark.

    The ``mean_ic`` and ``mean_hit_rate`` are equal-weight averages across the
    three models' out-of-sample fold statistics.  Live predictions (with
    BayesianRidge uncertainty) are generated by ``get_ensemble_signals()``.

    Attributes:
        benchmark:      ETF ticker.
        target_horizon: Forward return horizon in months.
        mean_ic:        Equal-weight mean IC across the three models.
...
def run_ensemble_benchmarks(
    ...
    """
    Train an ElasticNet + Ridge + BayesianRidge ensemble per ETF benchmark.

    For each ETF column in ``relative_return_matrix``, three independent WFO
    models are trained.  Out-of-sample IC, hit rate, and MAE are averaged
    (equal weight) across models.  Live ensemble predictions with BayesianRidge
    uncertainty are generated separately by ``get_ensemble_signals()``.
...
def get_ensemble_signals(
    ...
    For each ETF, calls ``predict_current()`` on each of the three model
    types, averages the point predictions (equal weight), and uses the
    BayesianRidge ``predict_with_std()`` for uncertainty quantification.
    ...
    for etf, ens_result in ensemble_results.items():
        ...
        for mtype, wfo_result in ens_result.model_results.items():
            try:
                ...
                point_preds.append(pred["predicted_return"])

                # Uncertainty from BayesianRidge
        ...
        point_prediction = float(np.mean(point_preds))
        signal_to_noise = (
