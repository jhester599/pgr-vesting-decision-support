...
def build_chart_frames(conn) -> dict[str, pd.DataFrame]:
    ...
    annual = annual_capital_return(monthly, dividends, inputs.prices)
...
def scatter_years(annual: pd.DataFrame) -> pd.DataFrame:
    ...
    full = annual[(annual["cr_months"] == 12) & annual["market_cap"].notna()]
    return full[(full.index >= 2005) & (full.index <= 2025)]
...
def plot_charts(frames: dict[str, pd.DataFrame], out_dir: str) -> list[str]:
    ...
    cbar.set_ticks([int(sc_years[0]), 2010, 2015, 2020, int(sc_years[-1])])
    fig3.tight_layout()
    _save(fig3, "pgr_cr_vs_capital_return.png")
    return written
...
def print_summary(frames: dict[str, pd.DataFrame]) -> None:
    ...
    for year, row in annual.iterrows():
        marker = " ★" if row["partial_label"] else ""
