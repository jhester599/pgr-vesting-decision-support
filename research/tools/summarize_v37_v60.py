...
Was ``results/research/summarize_all.py``. Run from the repository root:
``python research/tools/summarize_v37_v60.py``.
"""
...
STUDIES_DIR = Path("research") / "studies"


def main() -> None:
    """Aggregate experiment CSVs into one ranked table."""
    rows: list[dict[str, object]] = []
    for csv_path in sorted(STUDIES_DIR.glob("v[3-6][0-9]_*/outputs/v*results.csv"), key=lambda p: p.name):
        try:
