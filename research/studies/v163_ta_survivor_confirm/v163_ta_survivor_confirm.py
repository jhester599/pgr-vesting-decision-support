...
OUTPUT_DIR = Path("research") / "studies" / "v163_ta_survivor_confirm" / "outputs"
CONFIRM_PATH = OUTPUT_DIR / "v163_ta_survivor_confirm_summary.csv"
PAYLOAD_PATH = OUTPUT_DIR / "v163_ta_survivor_candidate.json"
# v162 writes its detail file to the gitignored outputs/detail/ (over 1 MB).
DETAIL_PATH = study_output_path("v162_ta_broad_screen_detail.csv", detail=True)
...
def run_survivor_confirmation(
    screen_summary_path: Path = study_output_path("v162_ta_broad_screen_summary.csv"),
    output_dir: Path = OUTPUT_DIR,
    screen_detail_path: Path = DETAIL_PATH,
) -> dict[str, Any]:
    """Run capped survivor selection from the v162 summary artifact.

    The benchmark-family slices also need v162's detail file, which is not
    committed; they are skipped when it has not been regenerated.
    """
    if not screen_summary_path.exists():
        ...
        detail_path = screen_detail_path
...
def main() -> None:
    ...
    parser.add_argument("--screen-summary", default=str(study_output_path("v162_ta_broad_screen_summary.csv")))
