...
OUTPUT_DIR = Path("research") / "studies" / "x23_dividend_lane_package" / "outputs"
...
def main() -> None:
    summary = build_x23_summary(
        {
            "x20": _read_json(study_output_path("x20_dividend_policy_synthesis.json")),
            "x21": _read_json(study_output_path("x21_dividend_target_scales_summary.json")),
            "x22": _read_json(study_output_path("x22_dividend_size_baselines_summary.json")),
        }
    )
