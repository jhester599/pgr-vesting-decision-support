...
OUTPUT_DIR = Path("research") / "studies" / "x20_dividend_policy_synthesis" / "outputs"
...
def main() -> None:
    x18 = _read_json(study_output_path("x18_dividend_policy_summary.json"))
    x10 = _read_json(study_output_path("x10_dividend_capital_summary.json"))
    x19 = _read_json(study_output_path("x19_post_policy_dividend_summary.json"))
    x10["detail_rows"] = _read_csv_rows(study_output_path("x10_dividend_capital_detail.csv"))
    x19["detail_rows"] = _read_csv_rows(study_output_path("x19_post_policy_dividend_detail.csv"))
    payloads = {
        "x18": x18.get("summary", {}),
        "x10": x10,
        "x19": x19,
