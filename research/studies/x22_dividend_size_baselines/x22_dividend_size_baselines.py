...
OUTPUT_DIR = Path("research") / "studies" / "x22_dividend_size_baselines" / "outputs"
...
def _x21_candidate_metrics() -> tuple[pd.DataFrame, pd.DataFrame]:
    payload = json.loads((study_output_path("x21_dividend_target_scales_summary.json")).read_text(encoding="utf-8"))
    detail = pd.read_csv(study_output_path("x21_dividend_target_scales_detail.csv"))
