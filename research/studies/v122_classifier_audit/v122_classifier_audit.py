...
def main() -> None:
    ...
    coeff_path = study_output_path("v122_classifier_audit_coefficients.csv")
    ...
    feature_totals_path = study_output_path("v122_classifier_audit_feature_totals.csv")
    feature_totals.to_csv(feature_totals_path, index=False)

    shadow_summary = summary["classification_shadow"]
    pooled_v90 = pd.read_csv(study_output_path("v90_pooled_panel_classifiers_results.csv"))
    pooled_best = pooled_v90.loc[
        pooled_v90["selected_next"].fillna(False).astype(bool)
    ].iloc[0]
    v92 = pd.read_csv(study_output_path("v92_calibration_and_abstention_results.csv"))
