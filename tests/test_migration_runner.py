...
def test_initialize_schema_applies_migration_on_fresh_db(tmp_path: Path) -> None:
    ...
    assert version == "006_pgr_fundamentals_quarterly_rebuild"
...
def test_initialize_schema_is_idempotent(tmp_path: Path) -> None:
    ...
    assert rows == [
        ...
        "005_pgr_edgar_monthly_raw",
        "006_pgr_fundamentals_quarterly_rebuild",
    ]
...
def test_list_migrations_includes_python_migrations_in_order() -> None:
    ...
    assert "005_pgr_edgar_monthly_raw" in ids
    assert "006_pgr_fundamentals_quarterly_rebuild" in ids
