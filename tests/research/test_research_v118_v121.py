...
def test_v118_results_have_expected_columns() -> None:
    df = pd.read_csv(study_output_path("v118_prospective_shadow_replay_results.csv"))
    assert {
        "date",
        "live_hold_fraction",
...
def test_v119_v121_outputs_have_expected_columns() -> None:
    v119 = pd.read_csv(study_output_path("v119_disagreement_scorecard_results.csv"))
    assert {
        "selected_variant",
        "agreement_rate",
        "disagreement_months",
    ...
    v120 = pd.read_csv(study_output_path("v120_prospective_shadow_gate_assessment_results.csv"))
    assert {
        "selected_variant",
        "decision",
        "agreement_pass",
    ...
    v121 = pd.read_csv(study_output_path("v121_prospective_shadow_phase_summary_results.csv"))
    assert {
        "selected_variant",
        "agreement_rate",
        "next_step_decision",
